...
**Goal:** Rewrite the project README into an evaluator-focused document that maps the PDF requirements to implementation, explains the architecture and design decisions, and provides runnable commands.

**Architecture:** This is a documentation change. `README.md` becomes the primary evaluator-facing document, while `.env.example` is aligned with the current runtime behavior so the run instructions do not contradict the code.

**Tech Stack:** Markdown, Python 3.13, FastAPI, Pydantic Settings, OpenAI SDK, pytest.

---

## File Structure

- Modify: `README.md` - replace the current README with the evaluator-focused structure from `docs/superpowers/specs/2026-04-27-readme-design.md`.
- Modify: `.env.example` - remove stale `LLM_PROVIDER=fake` and document that the production app requires `OPENAI_API_KEY`; tests inject `FakeAI` directly.

## Context Notes

Current runtime behavior:

- `src/workflow_engine/config.py` declares `openai_api_key: str` with no default.
- `src/workflow_engine/main.py` creates `Settings()` before building the app.
- `src/workflow_engine/bootstrap.py:build_dependencies()` always creates `OpenAIAdapter` for classify/generate tasks.
- `src/workflow_engine/bootstrap.py:build_test_dependencies()` injects `FakeAI` for tests.

Therefore README must not claim that the production server can start in fake mode without `OPENAI_API_KEY`. The accurate wording is:

- Production/local API server: `OPENAI_API_KEY` is required.
- Automated tests: no OpenAI call; `FakeAI` is injected through test dependencies.

## Task 1: Align `.env.example` With Runtime Settings
...
Rationale: `LLM_PROVIDER` is not consumed by current code. Leaving it in the example suggests a fake runtime mode that does not exist in `build_dependencies()`.

- [ ] **Step 2: Verify settings fields match the example**

Run:

```bash
rg -n "mock_api_base_url|mock_api_key|openai_api_key|openai_model|openai_classify_model|openai_generate_model|openai_temperature|llm_provider" src/workflow_engine/config.py .env.example src/workflow_engine/bootstrap.py
```

Expected:

- `.env.example` contains no `LLM_PROVIDER`.
- `config.py` contains `openai_api_key`, model fields, and mock API fields.
- `bootstrap.py` has no `llm_provider` branch.

- [ ] **Step 3: Commit**

```bash
git add .env.example
git commit -m "docs: align env example with runtime settings"
```
...
- category별 필수 포함 항목은 substring 기반으로 검증합니다.

이 검증은 LLM 응답을 완전히 의미적으로 보장하지는 않지만, 과제 MVP에서는 PDF 정책을 prompt와 코드 검증 양쪽에 고정하는 데 초점을 맞췄습니다.
...
- `generate_reply`의 필수 포함 항목 검증은 substring 기반입니다. 의미 검증이 필요하면 별도 LLM judge나 rule engine이 필요합니다.
- 등록된 workflow는 `customer_support_auto_reply` 하나입니다.
- multi-provider는 `AI` protocol로 확장 지점만 준비했고, 실제 adapter는 OpenAI만 구현했습니다.
````

- [ ] **Step 2: Verify README no longer contains stale fake runtime wording**

Run:

```bash
rg -n "LLM_PROVIDER|fake mode|OPENAI_API_KEY.*미설정|키 없이|fake runtime" README.md
```

Expected:

- No `LLM_PROVIDER`.
- No claim that the API server starts without `OPENAI_API_KEY`.
- It is acceptable for README to mention `FakeAI` in the testing section.

- [ ] **Step 3: Commit**

```bash
git add README.md
git commit -m "docs: rewrite README for assignment evaluation"
```
...
- [ ] **Step 1: Check README section order**
...
Expected headings in this order:
...
- [ ] **Step 2: Check runtime env wording**

Run:

```bash
rg -n "OPENAI_API_KEY|FakeAI|LLM_PROVIDER" README.md .env.example src/workflow_engine/config.py src/workflow_engine/bootstrap.py
```

Expected:

- README says production server requires `OPENAI_API_KEY`.
- README says tests use `FakeAI`.
- `.env.example` has `OPENAI_API_KEY` and no `LLM_PROVIDER`.
- `config.py` has no `llm_provider`.
- `bootstrap.py` has no `llm_provider`.
...
If the exact test count changes because unrelated user changes are present, all tests must still pass.
...
- Only README and `.env.example` documentation changes.
- No source code changes.
- README matches the evaluator-focused structure.

- [ ] **Step 5: Commit verification note if needed**

No commit is needed if Tasks 1 and 2 already committed the documentation changes. If Step 4 reveals a small documentation correction, commit it:

```bash
git add README.md .env.example
git commit -m "docs: polish README verification details"
```
...
- The plan implements the README section order from `docs/superpowers/specs/2026-04-27-readme-design.md`.
- The plan adjusts the spec's fake-mode wording to match the current code, where production runtime requires `OPENAI_API_KEY` and tests use `FakeAI`.
- The plan keeps the architecture centered on `main -> bootstrap -> engine/nodes/adapters`.
- The plan includes requirements mapping, run instructions, API examples, workflow definition, architecture, design decisions, LLM policy, retry, testing, security, and tradeoffs.

Known scope adjustment:

- `.env.example` is included because the current file contains `LLM_PROVIDER=fake`, which contradicts current runtime code and would make the README instructions misleading.
