...
|---|---|---|
| JSON 또는 YAML workflow 정의 | YAML로 workflow를 정의하고 Pydantic model로 로드합니다. | `workflows/customer_support_auto_reply.yaml`, `src/engine/loader.py`, `src/domain/workflow.py` |
| `DAG` 기반 실행 순서 결정 및 순환 검사 | `depends_on`으로 graph를 만들고 topological sort 중 cycle을 감지합니다. | `src/engine/validator.py` |
| 순차 실행 지원 | topological order를 따라 node를 하나씩 실행합니다. | `src/engine/executor.py` |
| node 간 `context` 전달 | 성공한 node 출력은 `context.nodes.<key>`에 저장되고 후속 node input mapping에서 참조됩니다. | `src/engine/executor.py`, `src/engine/input_mapping.py` |
| 오류 발생 시 `Exponential Backoff` retry | YAML에 선언된 노드 retry 정책에 따라 executor가 호출을 감쌉니다. | `src/engine/retry.py`, `src/engine/executor.py`, `workflows/customer_support_auto_reply.yaml` |
| `LLM Function Calling` 패턴 추상화 | `Tool`과 `LLM task`를 workflow node로 추상화하고, Tool 결과를 후속 LLM node에 `context`로 피드백합니다. | `src/engine/ports.py`, `src/engine/registries.py`, `src/nodes/llm.py`, `src/nodes/tools.py` |
| 표준화된 `Tool` 인터페이스 | 모든 Tool은 `execute(input_data) -> dict` 계약을 따릅니다. | `src/engine/ports.py`, `src/nodes/tools.py` |
| 상용 LLM API 연동 | OpenAI Chat Completions API를 JSON 응답 모드로 호출합니다. | `src/adapters/openai.py` |
| Mock CRM / Email API 통합 | Inquiry, CRM, Email API를 Bearer token과 함께 호출합니다. | `src/adapters/mock_api.py` |
| Human-in-the-Loop 승인 node | 승인 대기 상태에서 run을 pause하고 승인/거부 후 resume 또는 종료합니다. | `src/engine/executor.py`, `src/api/routes.py` |
| 승인 timeout 처리 | `ApprovalTimer`로 능동 만료를 처리하고, 조회 시 lazy fallback도 수행합니다. | `src/engine/approval_timer.py`, `src/engine/executor.py` |
...
과제의 맞춤형 응답 조건은 `src/domain/reply_policy.py`와 `src/nodes/prompts.py`에 반영했습니다.
