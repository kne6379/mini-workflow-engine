...
|---|---|---|
| JSON 또는 YAML workflow 정의 | YAML로 workflow를 정의하고 Pydantic model로 로드합니다. | `workflows/customer_support_auto_reply.yaml`, `workflow_engine/engine/loader.py`, `workflow_engine/domain/workflow.py` |
| `DAG` 기반 실행 순서 결정 및 순환 검사 | `depends_on`으로 graph를 만들고 topological sort 중 cycle을 감지합니다. | `workflow_engine/engine/validator.py` |
| 순차 실행 지원 | topological order를 따라 node를 하나씩 실행합니다. | `workflow_engine/engine/executor.py` |
| node 간 `context` 전달 | 성공한 node 출력은 `context.nodes.<key>`에 저장되고 후속 node input mapping에서 참조됩니다. | `workflow_engine/engine/executor.py`, `workflow_engine/engine/input_mapping.py` |
| 오류 발생 시 `Exponential Backoff` retry | YAML에 선언된 노드 retry 정책에 따라 executor가 호출을 감쌉니다. | `workflow_engine/engine/retry.py`, `workflow_engine/engine/executor.py`, `workflows/customer_support_auto_reply.yaml` |
| `LLM Function Calling` 패턴 추상화 | `Tool`과 `LLM task`를 workflow node로 추상화하고, Tool 결과를 후속 LLM node에 `context`로 피드백합니다. | `workflow_engine/engine/ports.py`, `workflow_engine/engine/registries.py`, `workflow_engine/nodes/llm.py`, `workflow_engine/nodes/tools.py` |
| 표준화된 `Tool` 인터페이스 | 모든 Tool은 `execute(input_data) -> dict` 계약을 따릅니다. | `workflow_engine/engine/ports.py`, `workflow_engine/nodes/tools.py` |
| 상용 LLM API 연동 | OpenAI Chat Completions API를 JSON 응답 모드로 호출합니다. | `workflow_engine/adapters/openai.py` |
| Mock CRM / Email API 통합 | Inquiry, CRM, Email API를 Bearer token과 함께 호출합니다. | `workflow_engine/adapters/mock_api.py` |
| Human-in-the-Loop 승인 node | 승인 대기 상태에서 run을 pause하고 승인/거부 후 resume 또는 종료합니다. | `workflow_engine/engine/executor.py`, `workflow_engine/api/routes.py` |
| 승인 timeout 처리 | `ApprovalTimer`로 능동 만료를 처리하고, 조회 시 lazy fallback도 수행합니다. | `workflow_engine/engine/approval_timer.py`, `workflow_engine/engine/executor.py` |
...
과제의 맞춤형 응답 조건은 `workflow_engine/domain/reply_policy.py`와 `workflow_engine/nodes/prompts.py`에 반영했습니다.
...
Retry 정책은 workflow YAML에 노드 단위로 선언합니다. 명시한 노드만 retry되고, 생략한 노드는 한 번만 시도합니다.
...
`max_attempts`만 노출하며, backoff 곡선 (`initial_delay_seconds=0.5`, `multiplier=2.0`, `max_delay_seconds=5.0`)은 `WorkflowExecutor.default_retry_policy`의 default를 사용합니다. 자동 테스트는 sleep 0인 빠른 정책을 주입합니다.

retry는 호출 + 응답 검증만 감쌉니다. `render_inputs` 같은 결정적 단계의 실패 (`InputMappingError`)는 retry budget을 소비하지 않고 즉시 `FAILED`로 떨어집니다.

`RetryExecutor`는 모든 예외를 잡습니다. 401 같은 영구적 에러도 `max_attempts`만큼 시도 후 같은 예외로 실패합니다. "재시도해봤자 같은 결과"인 경우 retry 블록을 생략하면 됩니다.

| 노드 | retry | 이유 |
|---|---|---|
| `fetch_inquiry` | 없음 | mock GET inquiry는 안정적 |
| `classify_inquiry` | `max_attempts: 3` | OpenAI rate limit / 일시 5xx 대비 |
| `lookup_customer` | 없음 | mock POST CRM lookup은 안정적 |
| `generate_reply` | `max_attempts: 3` | OpenAI rate limit / 일시 5xx 대비 |
| `wait_for_approval` | 없음 | human_approval은 retry inert (적어도 무시됨) |
| `send_reply_email` | `max_attempts: 5` | mock email API가 10% 확률로 503 |

retry 소진 시 run과 해당 node는 `FAILED`로 기록됩니다.
